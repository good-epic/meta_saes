...
@st.cache_resource
def load_model_and_sae_and_data():
    layer = 5
    device = 'cuda'
...
@st.cache_resource
def load_feature_statistics(filename, _sae):
    return FeatureStatistics.load(filename, _sae)

# Load the AutoInterpreter
@st.cache_resource
def load_interpreter(_stats, _model, _dataset):
    interpreter = AutoInterpreter(_stats, _model, _dataset)
    try:
...
def format_example(example, model):
    ...
    token = html.escape(model.to_string(example['token']))
...
def main():
    ...
    stats = load_feature_statistics("feature_stats_gemma_2b_l5_1m.pth", sae)
    interpreter = load_interpreter(stats, model, dataset)
    ...
    if st.session_state.page == "Feature Explorer":
        ...
        top_examples = stats.get_top_examples(st.session_state.feature_idx, model, dataset, top_k=10)
        for example in top_examples:
            st.markdown(format_example(example, model), unsafe_allow_html=True)
        ...
        if st.session_state.meta_feature_idx not in stats.cluster_to_features:
            ...
            for feature_idx, activation in features[:top_k]:
                ...
                # Display top examples
                st.write("### Top Examples:")
                top_examples = stats.get_top_examples(feature_idx, model, dataset, top_k=5)
                for example in top_examples:
                    st.markdown(format_example(example, model), unsafe_allow_html=True)
                    st.markdown("", unsafe_allow_html=True)
    ...
    # Add a button to save all descriptions at the bottom of the page
    if st.button("Save All Descriptions"):
        interpreter.save_descriptions("feature_descriptions.json")
        st.success("All descriptions saved successfully!")
