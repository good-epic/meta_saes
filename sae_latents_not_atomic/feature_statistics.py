...
class FeatureStatistics():
    ...
    def process_activating_examples(self, sae, model, dataset, batch_size=2):
        ...
        for batch_idx in tqdm(range(num_batches)):
            ...
            active_indices = torch.nonzero(mask, as_tuple=True)
            batch_nums, seq_positions, feature_indices = active_indices
    ...
    @torch.no_grad()
    def process_clusters(self, sae, meta_sae):
        self.cluster_to_features = defaultdict(list)
        self.feature_to_clusters = defaultdict(list)
    ...
    @torch.no_grad()
    def get_top_examples(self, feature_idx, model, dataset, top_k=10, pre_context=10, post_context=6):
...
if __name__ == "__main__":
    ...
    from meta_saes.sae import BatchTopKSAE,load_gemma_sae, load_wandb_sae

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    layer = 5
    model, saes, dataset = load_gemma_sae(
        sae_id=f"layer_{layer}/width_1m/canonical",
        # sae_id=f"layer_{layer}/width_16k/canonical",
        # release="gemma-scope-2b-pt-res",
        # sae_id=f"layer_{layer}/width_16k/average_l0_68",
        device=device,
    )
    model = model.to(torch.float16)
    base_sae = saes[0].to(device).eval().to(torch.float16)
    meta_sae, cfg = load_wandb_sae(
        f"patrickaaleask/gemma-scope-2b-pt-res-canonical-meta-saes/gemma-2-2b_blocks.{layer}.hook_resid_post_32768_topk_32_0.0001_262432:v0",
        # f"patrickaaleask/gemma-scope-2b-pt-res-canonical-meta-saes/gemma-2-2b_blocks.{layer}.hook_resid_post_8192_topk_8_0.001_19001:v0",
        # "patrickaaleask/gemma-scope-2b-pt-res-meta-saes/gemma-2-2b_blocks.5.hook_resid_post_2304_topk_8_0.0001_15001:v0",
        BatchTopKSAE,
    )
    dataset = torch.cat([dataset[i]["tokens"].unsqueeze(0) for i in range(dataset.shape[0])], dim=0)
...
if __name__ == "__main__":
    ...
    stats.process_activating_examples(base_sae, model, dataset, batch_size=1)
    stats.save("feature_stats_gemma_2b_l5_1m.pth")

    stats.process_clusters(base_sae, meta_sae)
    stats.save("feature_stats_gemma_2b_l5_1m.pth")
